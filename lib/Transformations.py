from pyspark.sql.functions import (struct, lit, col, array, when, isnull, filter, current_timestamp, date_add,
                                   collect_set, collect_list, expr, date_format)
...
def apply_header(spark, df):
    ...
    header_df = spark.createDateFrame(header_info) \
                .toDF("eventType", "majorSchemaVersion", "minorSchemaVersion")
    event_df = header_df.hint("broadcast").crossJoin(df) \
        .select(struct(expr("uuid()").alias("evenIdentifier"),
                       col("eventType"), col("majorSchemaVersion"), col("minorSchemaVersion"),
                       lit(date_format(current_timestamp(), "yyyy-MM-dd 'T' HH:mm:ssZ")).alias("evenDateTime")
                       ).alias("evenHeader"),
